...
class Order(models.Model):
    ...
    payment_option = models.CharField('Opção de Pagamento',choices=PAYMENT_CHOICES,max_length=20)
    ...
    def pagseguro_update_status(self,status):
        if status =='3': #concluida
            self.status = 1
        elif status == '7': #cancelada
            self.status = 2
        self.save()
...
def post_cart_item_save(instance, **kwargs):
    if instance.quantity < 1:
        instance.delete()

models.signals.post_save.connect(post_cart_item_save,sender=CartItem,dispatch_uid='post_cart_item_save')
